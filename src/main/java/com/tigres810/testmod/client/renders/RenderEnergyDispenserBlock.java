...
public class RenderEnergyDispenserBlock extends TileEntityRenderer<TileEnergyDispenserBlock>
{
    ...
    public RenderEnergyDispenserBlock(TileEntityRendererDispatcher dispatcher) { super(dispatcher); }

    @Override
    public void render(TileEnergyDispenserBlock te, float partialTicks, MatrixStack mstack, IRenderTypeBuffer buffer, int light, int overlay)
    {
        if(te.isconnected) {
    ...
    @Override
    public boolean shouldRenderOffScreen(TileEnergyDispenserBlock te) { return true; }
}