...
public class RenderPlantEnergizer extends TileEntityRenderer<TilePlantEnergizerBlock> {

	public RenderPlantEnergizer(TileEntityRendererDispatcher rendererDispatcherIn) {
		super(rendererDispatcherIn);
	}

	@Override
	public void render(TilePlantEnergizerBlock tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
	...
	public static final ResourceLocation BLACK = new ResourceLocation("tmod:textures/blocks/texture3.png");
	
	public static void renderCubeUsingQuads(TilePlantEnergizerBlock tileEntityIn, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer renderBuffers, int combinedLight, int combinedOverlay) {
		...
		drawCubeQuads(matrixStack, renderBuffers, gemColour, combinedLight, tileEntityIn);
	...
	*/
	private static void drawCubeQuads(MatrixStack matrixStack, IRenderTypeBuffer renderBuffer, Color color, int combinedLight, TilePlantEnergizerBlock tileEntityIn) {
		tileEntityIn.getCapability(CapabilityEnergy.ENERGY).ifPresent(h -> {
			if(h.getEnergyStored() > 0) {
				CURRENTTEXTURE = TEXTURE;
			} else {
				CURRENTTEXTURE = BLACK;
			}
		});
		IVertexBuilder vertexBuilderBlockQuads = renderBuffer.getBuffer(RenderType.entityTranslucent(CURRENTTEXTURE));
		...
		matrixStack.translate(-.5, 1.15f, -.5);
