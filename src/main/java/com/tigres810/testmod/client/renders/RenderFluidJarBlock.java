...
public class RenderFluidJarBlock extends TileEntityRenderer<TileFluidJarBlock> {

	public RenderFluidJarBlock(TileEntityRendererDispatcher rendererDispatcherIn) {
		super(rendererDispatcherIn);
	}

	@Override
	public void render(TileFluidJarBlock tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
		renderCubeUsingQuads(tileEntityIn, partialTicks, matrixStackIn, bufferIn, combinedLightIn, combinedOverlayIn);
	}
	
	public static final ResourceLocation TEXTURE = new ResourceLocation("tmod:textures/blocks/white_fluid.png");
	
	public static void renderCubeUsingQuads(TileFluidJarBlock tileEntityMBE21, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer renderBuffers, int combinedLight, int combinedOverlay) {
	...
	*/
	private static void drawCubeQuads(MatrixStack matrixStack, IRenderTypeBuffer renderBuffer, Color color, int combinedLight, TileFluidJarBlock tileEntityIn) {
FluidStack fluid = tileEntityIn.getTank().getFluid();
        ...
        if(fluidStill != null) {
			...
			IVertexBuilder vertexBuilderBlockQuads = renderBuffer.getBuffer(RenderType.entityTranslucent(new ResourceLocation(spriteLocation.getNamespace(), "textures/" + spriteLocation.getPath() + ".png")));
