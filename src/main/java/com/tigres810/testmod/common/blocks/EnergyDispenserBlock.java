...
public class EnergyDispenserBlock extends Block implements IPipeConnect {
	...
	public EnergyDispenserBlock(Properties properties) {
		super(properties);
	}
	...
	public void playerWillDestroy(World worldIn, BlockPos pos, BlockState state, PlayerEntity placer) {
		...
		if(te instanceof TileEnergyDispenserBlock) {
			if(((TileEnergyDispenserBlock) te).isconnected) {
				TileEntity anotherte = worldIn.getBlockEntity(((TileEnergyDispenserBlock) te).getConnectedTo());
				
				if(anotherte instanceof TileEnergyDispenserBlock) {
					((TileEnergyDispenserBlock) anotherte).setConnectedTo(BlockPos.ZERO, true, false);
				}
			}
			if(((TileEnergyDispenserBlock) te).isfakeconnected) {
				TileEntity anotherte = worldIn.getBlockEntity(((TileEnergyDispenserBlock) te).getFakeConnected());
				
				if(anotherte instanceof TileEnergyDispenserBlock) {
					((TileEnergyDispenserBlock) anotherte).setConnectedTo(BlockPos.ZERO, false, false);
				}
			}
		}
	...
	public void setPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
		if(!worldIn.isClientSide()) {
			...
			if(te != null) {
				if (te instanceof TileFluidTankBlock || te instanceof TilePlantEnergizerBlock) {
	...
	public TileEntity createTileEntity(BlockState state, IBlockReader world) {
		return TileEntityInit.ENERGYDISPENSER_BLOCK_TILE.get().create();
	}
